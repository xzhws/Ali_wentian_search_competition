...
def simcse_sup_loss(y_pred: 'tensor') -> 'tensor':
    ...
    为什么这里的label要这么设计呢
    '''
    y_true = torch.arange(y_pred.shape[0], device=DEVICE) # [batch_size*3]
    use_row = torch.where((y_true + 1) % 3 != 0)[0]
    y_true = (use_row - use_row % 3 * 2) + 1
    # batch内两两计算相似度, 得到相似度矩阵(对角矩阵)
    sim = F.cosine_similarity(y_pred.unsqueeze(1), y_pred.unsqueeze(0), dim=-1) # [batch_size * 3, 1,768] ,[1,batch_size*3,768]
    ...
    print('sim size():{}\n y_true size():{}'.format(sim.size(), y_true.size())) # [32, 48] [32]两部分
