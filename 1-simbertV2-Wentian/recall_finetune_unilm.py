...
# 基本信息 需要改动
epochs = 100
save_dir = "./output/res_finetune_unsupervised_100w_100_epoch/"
...
# mcpr_label_data_path = "/home/zqxie/project/SimCSE-Chinese-Pytorch-main/data/unsupervised_data_random_cut_random_rate.txt"
...
def corpus():
    ...
    while True:
        ...
        with open(dev_data_path, "r", encoding="utf8") as fr:
            t = [line.strip().split("\t") for line in fr]
            for _ in range(1): data.extend(t)
        with open(mcpr_label_data_path, "r", encoding="utf8") as fr:
            t = [line.strip().split("\t") for line in fr]
            for _ in range(1): data.extend(t)
